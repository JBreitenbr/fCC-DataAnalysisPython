...
def calculate_demographic_data(print_data=True):
    ...
    percentage_bachelors = round(100*len(df[df["education"]=="Bachelors"
  ])/len(df),1)
    ...
    g_high=high.groupby(["native-country"])["salary"].count()
    g_all=df.groupby(["native-country"])["salary"].count()                 
    countries=pd.merge(g_high,g_all,on="native-country")
    countries["high_perc"]=round(100*countries["salary_x"]/countries["salary_y"],1)
    highest_earning_country =(countries["high_perc"]).idxmax()
    highest_earning_country_percentage=countries["high_perc"].loc[highest_earning_country]
